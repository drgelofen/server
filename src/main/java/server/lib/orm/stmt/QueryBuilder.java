...
import server.lib.orm.field.ForeignCollectionField;
import server.lib.orm.stmt.query.ColumnNameOrRawSql;
import server.lib.orm.stmt.query.OrderBy;
...
public class QueryBuilder<T, ID> extends StatementBuilder<T, ID> {
     ...
     */
//    TODO: MODIFIED
    public QueryBuilder<T, ID> leftJoin(String localColumnName, String joinedColumnName,
                                        QueryBuilder<?, ?> joinedQueryBuilder) throws SQLException {
        addJoinInfo(JoinType.LEFT, localColumnName, joinedColumnName, joinedQueryBuilder, JoinWhereOperation.AND);
        return this;
    }

//    TODO: MODIFIED
    public QueryBuilder<T, ID> innerJoin(String localColumnName, String joinedColumnName,
                                         QueryBuilder<?, ?> joinedQueryBuilder) throws SQLException {
    ...
    protected void appendStatementStart(StringBuilder sb, List<ArgumentHolder> argList) {
        ...
        if (joinList != null) {
            ...
            for (char c : sb.toString().toCharArray()) {
                if (c == '*') {
                    reformQuery(sb, sb.toString().substring(0, index));
                    return;
                }
                index++;
            }
...
//	TODO: MODIFIED
    private void reformQuery(StringBuilder sb, String sub) {
        StringBuilder builder = new StringBuilder();
        int index = sub.length();
        boolean append = false;
        while (index > 0) {
            char c = sub.charAt(--index);
            if (c == ' ') {
                append = true;
            }
            if (append) {
        ...
        sb.replace(0, sub.length(), builder.toString());
    }
...
//	TODO: MODIFY
	protected void appendTableQualifier(StringBuilder sb) {
		if (tableInfo.getTableAlias() != null) {
			databaseType.appendEscapedEntityName(sb, tableInfo.getTableAlias());
		} else {
			if (tableInfo.getSchemaName() != null && tableInfo.getSchemaName().length() > 0) {
				databaseType.appendEscapedEntityName(sb, tableInfo.getSchemaName());
				sb.append('.');
    ...
    private void appendJoinSql(StringBuilder sb) {
        for (JoinInfo joinInfo : joinList) {
            if (joinInfo.queryBuilder.tableInfo.getTableName().equals(tableInfo.getTableName())) {
                joinInfo.queryBuilder.tableInfo = joinInfo.queryBuilder.dao.initTableInfo();
            }
            tableInfo.setTableAlias("X");
            sb.append(" AS ").append(tableInfo.getTableAlias()).append(" ");
            ...
            joinInfo.queryBuilder.tableInfo.setTableAlias("Y");
            sb.append(" AS ").append(joinInfo.queryBuilder.tableInfo.getTableAlias());
            sb.append(" ON ");
