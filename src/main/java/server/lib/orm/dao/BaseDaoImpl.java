...
import server.lib.orm.misc.BaseDaoEnabled;
...
import server.lib.orm.stmt.SelectArg;
import server.lib.orm.stmt.SelectIterator;
import server.lib.orm.stmt.StatementBuilder.StatementType;
import server.lib.orm.stmt.StatementExecutor;
...
import server.lib.orm.table.DatabaseTableConfig;
import server.lib.orm.table.ObjectFactory;
...
//	TODO: MODIFIED
    @Override
    public TableInfo initTableInfo() {
        try {
            if (tableConfig == null) {
                return new TableInfo<T, ID>(databaseType, dataClass);
            } else {
                tableConfig.extractFieldTypes(databaseType);
                return new TableInfo<T, ID>(databaseType, tableConfig);
            }
        } catch (Throwable t) {
            t.printStackTrace();
        }
