...
import server.lib.orm.stmt.ColumnArg;
...
abstract class BaseComparison implements Comparison {
    ...
    public final SelectArg wrap(Object value) {
        if (value != null && !(value instanceof SelectArg)) {
            return new SelectArg(value);
        }
        return (SelectArg) value;
    }

    public final Iterable<SelectArg> wrap(Iterable<?> objects) {
        ArrayList<SelectArg> args = new ArrayList<>();
        for (Object value : objects) {
            if (value != null && !(value instanceof SelectArg)) {
                args.add(new SelectArg(value));
            } else {
                args.add((SelectArg) value);
            }
        }
        return args;
    }
