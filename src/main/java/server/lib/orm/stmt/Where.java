...
import server.lib.orm.stmt.query.Between;
import server.lib.orm.stmt.query.Clause;
import server.lib.orm.stmt.query.Exists;
import server.lib.orm.stmt.query.In;
import server.lib.orm.stmt.query.InSubQuery;
import server.lib.orm.stmt.query.IsNotNull;
import server.lib.orm.stmt.query.IsNull;
import server.lib.orm.stmt.query.ManyClause;
import server.lib.orm.stmt.query.NeedsFutureClause;
import server.lib.orm.stmt.query.Not;
import server.lib.orm.stmt.query.Raw;
import server.lib.orm.stmt.query.SimpleComparison;
...
//    TODO: MODIFIED
    public Where<T, ID> and(Where<T, ID>... others) {
        if (others == null || others.length <= 1) {
            return this;
        }
        Clause[] clauses = buildClauseArray(others, "AND");
        addClause(new ManyClause(null, null, clauses, ManyClause.Operation.AND));
        return this;
    }
...
//	TODO: MODIFIED
    public Where<T, ID> eqIgnoreCase(String columnName, Object value) throws SQLException {
        addClause(new SimpleComparison(columnName, findColumnFieldType(columnName), value,
                SimpleComparison.EQUAL_TO_OPERATION_IGN_CASE));
        return this;
    }
...
//	}
//  TODO: MODIFIED
    public Where<T, ID> or(Where<T, ID>... others) {
        if (others == null || others.length <= 1) {
            return this;
        }
        Clause[] clauses = buildClauseArray(others, "OR");
        addClause(new ManyClause(null, null, clauses, ManyClause.Operation.OR));
        return this;
    }
